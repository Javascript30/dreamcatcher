...
//   try {
//     const result = await pool.all('SELECT * FROM dreams ORDER BY created_at DESC');
//     res.json(result);
//   } catch (error) {
...
//     const result = await pool.get('SELECT * FROM dreams WHERE id = $1', req.params.id);
...
//       'INSERT INTO dreams (dream_text, interpretation) VALUES ($1, $2), RETURNING x',
//       [validation.value, interpretation]
//     );
    
//     // Fetch the inserted record
//     const dream = await pool.get('SELECT * FROM dreams WHERE id = ?', result.lastID);
...
//   try {
//     const result = await pool.run('DELETE FROM dreams WHERE id = ?', req.params.id);
